...
> a different account, update those two lines too, or the admin panel's save button
> won't be able to find the right repo.
...
## 5. Manually reassign a week (admin panel)

Sometimes someone can't do their turn — the site has a **⚙️ Manage roster** panel
at the bottom for that. It writes directly to `overrides.json` in your repo via the
GitHub API, so both the website *and* the next WhatsApp message reflect the change.
...
**Using the panel:**
1. Open the site, expand **⚙️ Manage roster**.
2. Pick the Sunday, choose the new room/toilet person (or "No toilet duty (skip)"),
   paste your token, click **Save override**.
3. To undo a manual change and go back to the normal rotation for that week, pick
   the week and click **Reset to rotation**.
